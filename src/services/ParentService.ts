...
const ParentService = {
  ...
  getNotes: async (student_id) => {
    try {
      const response = await axios.get(`${API_BASE_URL}/mov/notes`, {
        params: {
          student_id,
        },
      });
  ...
  insertUser: async (userData: Parent) => {
    try {
      const response = await axios.post<Parent>(
        `${API_BASE_URL}/users`,
        userData
      );
      return response.data;
  ...
  updateUser: async (userId: number, userData: Parent) => {
    try {
      const response = await axios.put<Parent>(
  ...
  deleteUser: async (userId: number) => {
    try {
      const response = await axios.delete(
        `${API_BASE_URL}/usersParent/${userId}`
      );
      return response.data;
    } catch (error) {
